...
# TO Functions
def main_to():
  current_date = datetime.now().strftime('%Y-%m-%d') # Sets current date for naming purposes
  ...
  # Cleans and aligns status inquiry dfs
...
def read_excel(radley_status_inquiry, forecast_13_week):
  ...
  df2 = pd.read_excel(forecast_13_week, skiprows=[0,1,2,3], engine = "openpyxl") # Skip rows on unedited excel , skiprows=[0,1,2,3]
...
def process_status_inquiry(df1):
  # Drops types like JIT and FIRM since the valuse in the 13w are Fcst
  df1 = df1[df1['Type'] == 'Fcst']

  # Converts date to datetime
  df1.loc[:,'Date'] = pd.to_datetime(df1['Date'].dt.date)

  # Renames the columns that will be compared later
  df1 = df1.rename(columns={'Div': 'Company', 'Part Number': 'Part #'})

  # Sets a multi-index so that when the table is pivoted the company is garuanteed to stay associated with the part number
    # Ensures comparison is accurate
  df1.set_index(['Company', 'Part #'], inplace = True)

  # Pivots table on the index. Fills the
  df1 = df1.pivot_table(columns='Date', values='Required',index = ['Company', 'Part #'], aggfunc='sum').fillna('No Val')
...
def compare_files(df1, df2):
  ...
  for index in common_indices:
    for column in common_columns:
      if df2_aligned.loc[index, column] != 'No Val' and df1_aligned.loc[index, column] != 'No Val':
      ...
      elif df2_aligned.loc[index, column] != 'No Val' or df1_aligned.loc[index, column] != 'No Val':
        df_differences.loc[index, column] = ('No Comparison' + 'RadStatIn: '+ str(df1_aligned.loc[index, column])+ ' 13W: '+str(df2_aligned.loc[index, column]))

  # Rows in df1 but not in df2
  df_extra_in_df1 = df1[~df1.index.isin(df2.index)]

  # Fill NaNs with False for clarity (optional)
  df_differences = df_differences.fillna('SAME')

  styled_df = df_differences.style.map(highlight_match).map(highlight_diff).map(highlight_no_comp)

  styled_df.to_excel(f'{current_date} {f_company} compared.xlsx')
